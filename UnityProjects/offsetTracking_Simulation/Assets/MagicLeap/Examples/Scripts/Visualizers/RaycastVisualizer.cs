...
namespace MagicLeap
{
    ...
    public class RaycastVisualizer : MonoBehaviour
    {
        ...
        [SerializeField, Tooltip("The desired size of the screen display.")]
        ...
        //private PlanesExample controllerReferent;
        private bool triggerPress = false;

        private GameObject screenBox = null;

        [SerializeField, Tooltip("The screen GameObject.")]
        ...
        void Awake()
        {
            ...
            //GameObject planesExample = GameObject.Find("planesExample");
            //controllerReferent = planesExample.GetComponent("PlanesExample") as PlanesExample;
        ...
        private void createScreenBox(Vector3 center, Vector3 normalVector, float size)
        {
            if(screenBox != null)
            {
                RaycastVisualizer.Destroy(screenBox);
            }

            screenBox = Instantiate(screen, center, Quaternion.LookRotation(normalVector, new Vector3(0,1,0)));
            //screen.SetActive(true);
            //screen.transform.position = center;
            //screen.transform.rotation = Quaternion.LookRotation(normalVector, new Vector3(0, 1, 0));
            screenBox.SetActive(true);
            //screenBox = Instantiate(screen, new Vector3(0,0,0), Quaternion.FromToRotation(transform.up, normalVector));
        }
        #endregion

        #region Event Handlers
        public void RaycastTriggerPress()
        {
            triggerPress = true;
        }
        ...
        public void activateScreen(bool input)
        {
            if (screenBox != null)
            {
                screenBox.SetActive(input);
            }
        }
        #endregion
        ...
        public void OnRaycastHit(MLWorldRays.MLWorldRaycastResultState state, RaycastHit result, float confidence)
        {
            if (state != MLWorldRays.MLWorldRaycastResultState.RequestFailed && state != MLWorldRays.MLWorldRaycastResultState.NoCollision)
            {
                ...
                if(triggerPress)
                {
                    ...
                    triggerPress = false;
                    createScreenBox(result.point, result.normal, desiredSize);
                }
